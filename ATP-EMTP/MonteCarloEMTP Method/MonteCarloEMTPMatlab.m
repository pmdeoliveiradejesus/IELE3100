...
Rg=20;%Resistencia de Impulso de Puesta a Tierra, ohms 
...
%%% Otros datos
BW = 11; % distancia entre cables de guarda m
%TW = 39.3; % altura de la torre
TW = 34.63; % altura de la torre menos la flecha del cable de guarda
...
for  h=1:length(w)   
     CFO(:,h) = 0.01*w(h) * (400 + 710 * (t/1000) .^ -0.75) * 1000*Kt; %Calcula el CFO in V
    end
kk=1;
for i = 1:D
...
% Imprimir por pantalla resultados de la tasa y la probabilidad para w1, w2, w3, w4, w5 ...
%insulat length; #faults #BF;       #Frequency; Probability; BFOR 
   for  h=1:length(w) 
[w(h)/100        kk-1   nfo(kk-1,h) RN        P(kk-1,h)   T(kk-1,h)]
end
%  save P
%  save T
%  % figure
